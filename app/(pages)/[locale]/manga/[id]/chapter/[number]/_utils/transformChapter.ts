...
export interface TransformedChapter extends Omit<ChapterQuery["chapter"], "images"> {
...
export const transformChapter = (chapter: ChapterQuery["chapter"]): TransformedChapter => {
  ...
  let maxImages = Math.max(...chapter.images.map(images => images.images.length));

  for (let i = 0; i < maxImages; i++) {
    images.push({} as MultiLanguageImage);
  }

  chapter.images.forEach(version => {
